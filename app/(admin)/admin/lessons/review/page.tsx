...
export default function ReviewLessonPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/lessons/review?lessonId=${encodeURIComponent(lessonId)}`)
      ...
      .then((j) => {
        ...
        // Initial preview from DB planner (no override by default)
        fetchPreview(false);
      })
      .catch((e) => {
        if (!mounted) return;
  ...
  async function onConfirm() {
    ...
    try {
      ...
      // After confirm we keep the last preview, but the real truth
      // is now DB allocations + DB hazards.
    } catch (e: any) {
  ...
  return (
    <Section
      title="Review lesson"
      subtitle="Inspect the DB-planned FIFO allocation preview, then confirm."
    >
      <p className="mt-2 text-xs">
        After confirming, view rows in{" "}
        <a
          className="underline"
          href={`/admin/allocations?lessonId=${encodeURIComponent(lessonId)}`}
        >
          Admin → Allocations
        </a>
        .
      ...
      <div className="mt-3 grid gap-3 text-sm sm:grid-cols-2">
        ...
        <div className="flex items-center gap-2">
          <span className="text-gray-500">Student:</span> {studentName}
        ...
        <label className="mb-2 flex items-center gap-2">
          <input
            ...
            onChange={(e) => {
              ...
              setOverride(checked);
              // Re-run planner preview with override flag

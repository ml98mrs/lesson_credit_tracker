...
function getStringParam(sp: SearchParamsObject, key: string): string {
  return typeof sp[key] === "string" ? (sp[key] as string) : "";
}

function parseFilters(sp: SearchParamsObject): Filters {
  return {
    deliveryFilter: getStringParam(sp, "delivery"),
    tierFilter: getStringParam(sp, "tier"),
    lengthFilter: getStringParam(sp, "length"),
    stateFilter: getStringParam(sp, "state"),
    policyFilter: getStringParam(sp, "policy"),
    studentFilter: getStringParam(sp, "student"),
    extRefFilter: getStringParam(sp, "extRef"),
    sortRemaining: getStringParam(sp, "sortRemaining"),
    sortAmount: getStringParam(sp, "sortAmount"),
  };
}

async function fetchInvoiceLots(sb: ReturnType<typeof getAdminSupabase>) {
  try {
    ...
    if (error) {
      return { lots: [] as CreditInvoiceLot[], errorMsg: error.message };
    }

    return { lots: data ?? [], errorMsg: null as string | null };
  } catch (e: unknown) {
    const message =
      e instanceof Error
        ? e.message
        : "Unknown error while loading credit invoices";
    return { lots: [] as CreditInvoiceLot[], errorMsg: message };
  }
}

async function fetchRemainingByLotId(
  sb: ReturnType<typeof getAdminSupabase>,
  lots: CreditInvoiceLot[],
) {
  ...
  try {
    ...
    // ignore; we'll just leave the map empty
  }
...
async function fetchStudentNames(
  ...
  if (lots.length === 0) return nameByStudentId;
...
function buildRows(
  lots: CreditInvoiceLot[],
  remainingByLotId: Map<string, number | null>,
...
function applyFilters(rows: InvoiceRow[], filters: Filters): InvoiceRow[] {
  let filtered = rows;
  ...
  if (stateFilter) {
    filtered = filtered.filter(({ lot }) => lot.state === stateFilter);
  }

  if (policyFilter) {
    filtered = filtered.filter(({ lot }) => lot.expiry_policy === policyFilter);
  }

  if (studentFilter.trim()) {
    const sf = normalize(studentFilter.trim());
    filtered = filtered.filter(({ studentName, lot }) => {
...
function InvoiceFiltersForm({ filters }: FiltersFormProps) {
  const {
    deliveryFilter,
    tierFilter,
    lengthFilter,
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-1">
        <label htmlFor="student" className="text-[11px] text-gray-500">
          Student
        </label>
        <input
          id="student"
      ...
      <div className="flex flex-col gap-1">
        <label htmlFor="extRef" className="text-[11px] text-gray-500">
      ...
      {/* Dropdown filters */}
      <div className="flex flex-col gap-1">
        <label htmlFor="delivery" className="text-[11px] text-gray-500">
      ...
      <div className="flex flex-col gap-1">
        <label htmlFor="tier" className="text-[11px] text-gray-500">
          Tier restriction
        </label>
        <select
          id="tier"
      ...
      <div className="flex flex-col gap-1">
        <label htmlFor="length" className="text-[11px] text-gray-500">
          Length restriction
        </label>
        <select
          id="length"
      ...
      <div className="flex flex-col gap-1">
        <label htmlFor="state" className="text-[11px] text-gray-500">
          State
        </label>
        <select
          id="state"
      ...
      <div className="flex flex-col gap-1">
        <label htmlFor="policy" className="text-[11px] text-gray-500">
          Expiry policy
        </label>
        <select
          id="policy"
      ...
      <div className="flex flex-col gap-1">
        <label htmlFor="sortAmount" className="text-[11px] text-gray-500">
          Amount
        </label>
        <select
          id="sortAmount"
...
export default async function Page({
  ...
  const { lots, errorMsg } = await fetchInvoiceLots(sb);
  ...
  const [remainingByLotId, nameByStudentId] = await Promise.all([
    fetchRemainingByLotId(sb, lots),
    fetchStudentNames(sb, lots),
  ]);

  const baseRows = buildRows(lots, remainingByLotId, nameByStudentId);
  const filteredRows = applyFilters(baseRows, filters);
  const sortedRows = applySorting(
    filteredRows,
    filters.sortRemaining,
    filters.sortAmount,
  );
  ...
  return (
    ...
    >
      <InvoiceFiltersForm filters={filters} />
      <InvoiceTable rows={sortedRows} />
    </Section>
  );
}

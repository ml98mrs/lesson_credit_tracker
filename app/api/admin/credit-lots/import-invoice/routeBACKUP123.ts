...
const InvoiceSchema = z.object({
  ...
  lengthRestriction: z.enum(['none', '60', '90', '120']).optional().default('none'),

  expiryDate: ISO_DATE.nullable().optional(),
  expiryPolicy: z.enum(['none', 'advisory', 'mandatory']).optional().default('none'),
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Map unique constraint to 409 (idempotent re-import)
    if (error) {
      const msg = String(error.message || '');
      const details = (error as any)?.details ? String((error as any).details) : '';
      if (msg.includes('uq_invoice_external_ref_per_student') || details.includes('uq_invoice_external_ref_per_student')) {
        return NextResponse.json(
          { error: 'Invoice already imported for this student (idempotent).', code: 'IDEMPOTENT' },
          { status: 409 }
        );
      }

...
CREATE OR REPLACE FUNCTION "public"."rpc_admin_create_student"("p_auth_user_id" "uuid", "p_full_name" "text", "p_preferred_name" "text", "p_timezone" "text", "p_tier" "public"."tier", "p_teacher_id" "uuid") RETURNS "uuid"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
...
begin
  ...
  -- Optional: sanity check that a profile row existed
  if not found then
    raise exception 'No profile row found for auth_user_id=%', p_auth_user_id;
  end if;
  ...
  -- 3) Optional student_teacher pairing
...
begin
  ...
  -- 1) Adjust overdraft lot so remaining becomes zero
  update public.credit_lots
     set minutes_granted   = v_after_granted,
         award_reason_code = coalesce(award_reason_code, p_award_reason_code)
   where id = v_overdraft_lot.id;

  -- 2) Log settlement as an award
  ...
  values (
    v_overdraft_lot.id,
    'overdraft_settled_award',
    v_actor,
    v_now,
    jsonb_build_object(
      'student_id', p_student_id,
      'settlement_mode', 'award',
      ...
      'note', coalesce(p_note, 'Overdraft settled as award')
    )
  );
...
CREATE OR REPLACE FUNCTION "public"."rpc_import_invoice"("p_student_id" "uuid", "p_external_ref" "text", "p_minutes_granted" integer, "p_amount_pennies" integer, "p_start_date" "date", "p_delivery_restriction" "text" DEFAULT NULL::"text", "p_tier_restriction" "text" DEFAULT NULL::"text", "p_length_restriction" "public"."length_cat" DEFAULT 'none'::"public"."length_cat", "p_expiry_date" "date" DEFAULT NULL::"date", "p_expiry_policy" "public"."expiry_policy" DEFAULT 'none'::"public"."expiry_policy", "p_lessons_per_month" integer DEFAULT NULL::integer, "p_duration_per_lesson_mins" integer DEFAULT NULL::integer, "p_buffer" numeric DEFAULT NULL::numeric) RETURNS "public"."credit_lots"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
BEGIN
  ...
  IF v_row.id IS NULL THEN
    -- Already exists: return existing row unchanged, log duplicate
    SELECT *
      INTO v_row
    FROM public.credit_lots
    WHERE student_id = p_student_id
    ...
    VALUES (
      ...
      jsonb_build_object(
        ...
        'input_minutes_granted', p_minutes_granted,
        'input_start_date', p_start_date,
        'input_amount_pennies', p_amount_pennies
      )
    );
...
ALTER FUNCTION "public"."rpc_import_invoice"("p_student_id" "uuid", "p_external_ref" "text", "p_minutes_granted" integer, "p_amount_pennies" integer, "p_start_date" "date", "p_delivery_restriction" "text", "p_tier_restriction" "text", "p_length_restriction" "public"."length_cat", "p_expiry_date" "date", "p_expiry_policy" "public"."expiry_policy", "p_lessons_per_month" integer, "p_duration_per_lesson_mins" integer, "p_buffer" numeric) OWNER TO "postgres";
...
begin
  ...
  -- Find the overdraft lot (you currently only ever have one per student)
  select *
    into v_overdraft_lot
  from public.credit_lots
  where student_id = p_student_id
  ...
  -- 2) Log a specific settlement event
  insert into public.credit_lot_events (
    credit_lot_id,
    event_type,
    actor_id,
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'previous_minutes_granted', v_before_granted,
      'new_minutes_granted', v_after_granted,
      'note', coalesce(p_note, 'Overdraft settled by invoice')
    )
  );
...
declare
  ...
  -- Centralised duration rules:
  -- If you ever allow shorter SNC lessons, adjust here.
  v_min_duration_regular constant integer := 10;
begin
  ------------------------------------------------------------------
  -- 0) Resolve teacher from auth.uid()
  --    (Teacher must be logged in; profile_id → teachers.id)
  ------------------------------------------------------------------
  select t.id
    into v_teacher_id
  ...
  -- Centralised minimum duration rule
  -- (non-SNC lessons must be at least v_min_duration_regular minutes)
  ...
  ------------------------------------------------------------------
  -- 2) Insert lesson row as PENDING
  --    • state      = 'pending'   (lifecycle only)
  --    • is_snc     = p_is_snc
  --    • snc_mode   = 'none'      (decided later at confirm time)
  --    • length_cat = 'none'      (teacher doesn’t think in packages)
  ------------------------------------------------------------------
...
begin
  ...
  ------------------------------------------------------------------
  -- 1b) Derive invoice month (Europe/London) and block if PAID
  ------------------------------------------------------------------
  -- Use London month boundaries to match teacher_invoices.month_start
  v_month_start :=
...
begin
  ...
  values (
    p_student_id,
    null, -- applies to entire overdraft sum
    'negative',
    v_to_write_off,
    p_reason_code,
    p_note,
...
begin
  ...
  values (
    p_student_id,
    null, -- no single lot; applies to entire overdraft sum
    'negative',
    v_to_write_off,
    p_reason_code,
    p_note,
...
CREATE TABLE IF NOT EXISTS "public"."credit_lot_events" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "credit_lot_events_type_check" CHECK (("event_type" = ANY (ARRAY['created'::"text", 'settle_overdraft'::"text", 'minutes_adjusted'::"text", 'lot_closed'::"text", 'lot_expired'::"text"])))
);
...
CREATE OR REPLACE VIEW "public"."v_allocation_delivery_hazards_raw" WITH ("security_invoker"='true') AS
...
CREATE OR REPLACE VIEW "public"."v_allocation_delivery_hazards" WITH ("security_invoker"='true') AS
...
CREATE OR REPLACE VIEW "public"."v_allocation_length_restriction_hazards_raw" WITH ("security_invoker"='true') AS
 WITH "base" AS (
         SELECT "a"."id" AS "allocation_id",
            "a"."lesson_id",
            "a"."credit_lot_id",
...
CREATE OR REPLACE VIEW "public"."v_allocation_length_restriction_hazards" WITH ("security_invoker"='true') AS
 SELECT "r"."allocation_id",
    "r"."lesson_id",
    "r"."credit_lot_id",
    "r"."hazard_type"
...
CREATE OR REPLACE VIEW "public"."v_student_names" WITH ("security_invoker"='true') AS
...
SELECT
    NULL::"uuid" AS "credit_lot_id",
    NULL::"uuid" AS "student_id",
    NULL::"text" AS "source_type",
    NULL::"text" AS "award_reason_code",
    NULL::"text" AS "external_ref",
    NULL::integer AS "minutes_granted",
    NULL::integer AS "minutes_allocated",
    NULL::integer AS "minutes_remaining",
    NULL::boolean AS "is_overdrawn",
    NULL::"public"."delivery" AS "delivery_restriction",
    NULL::"public"."tier" AS "tier_restriction",
    NULL::"public"."length_cat" AS "length_restriction",
    NULL::"date" AS "start_date",
    NULL::"public"."expiry_policy" AS "expiry_policy",
    NULL::"date" AS "expiry_date",
    NULL::integer AS "days_to_expiry",
    NULL::boolean AS "expiry_within_30d",
    NULL::"public"."credit_lot_state" AS "state",
...
CREATE OR REPLACE VIEW "public"."v_lesson_length_hazards_raw" WITH ("security_invoker"='true') AS
...
CREATE OR REPLACE VIEW "public"."v_lesson_length_hazards" WITH ("security_invoker"='true') AS
 SELECT "r"."lesson_id",
    "r"."student_id",
    "r"."teacher_id",
    "r"."duration_min",
...
CREATE OR REPLACE VIEW "public"."v_overdraft_allocation_hazards" WITH ("security_invoker"='true') AS
 SELECT "a"."lesson_id",
...
CREATE OR REPLACE VIEW "public"."v_snc_overuse_hazards_raw" WITH ("security_invoker"='true') AS
 WITH "monthly_snc" AS (
         SELECT "lessons"."student_id",
            ("date_trunc"('month'::"text", "lessons"."occurred_at"))::"date" AS "month_start",
            "count"(*) AS "snc_count"
           FROM "public"."lessons"
          WHERE (("lessons"."is_snc" = true) AND ("lessons"."state" = 'confirmed'::"public"."lesson_state"))
          GROUP BY "lessons"."student_id", ("date_trunc"('month'::"text", "lessons"."occurred_at"))
        ), "overuse" AS (
         SELECT "monthly_snc"."student_id",
            "monthly_snc"."month_start",
            "monthly_snc"."snc_count"
           FROM "monthly_snc"
          WHERE ("monthly_snc"."snc_count" > 3)
        )
 SELECT "l"."id" AS "lesson_id",
  ...
  WHERE (("l"."is_snc" = true) AND ("l"."state" = 'confirmed'::"public"."lesson_state"));


ALTER VIEW "public"."v_snc_overuse_hazards_raw" OWNER TO "postgres";


CREATE OR REPLACE VIEW "public"."v_snc_overuse_hazards" WITH ("security_invoker"='true') AS
 SELECT "r"."lesson_id",
    "r"."student_id",
    "r"."teacher_id",
    "r"."month_start",
...
CREATE OR REPLACE VIEW "public"."v_lesson_hazards" WITH ("security_invoker"='true') AS
 SELECT "r"."lesson_id",
...
CREATE OR REPLACE VIEW "public"."v_lesson_revenue_detail" WITH ("security_invoker"='true') AS
 SELECT "a"."lesson_id",
...
CREATE OR REPLACE VIEW "public"."v_student_teacher_rate_summary" WITH ("security_invoker"='true') AS
 WITH "base" AS (
...
CREATE OR REPLACE VIEW "public"."v_teacher_lesson_earnings_detail" WITH ("security_invoker"='true') AS
 SELECT "l"."id" AS "lesson_id",
    "l"."teacher_id",
    "l"."student_id",
    ...
    "str"."student_tier",
        CASE
            WHEN ("l"."delivery" = 'online'::"public"."delivery") THEN "str"."effective_online_rate_pennies"
            WHEN ("l"."delivery" = 'f2f'::"public"."delivery") THEN "str"."effective_f2f_rate_pennies"
            ELSE NULL::integer
        END AS "hourly_rate_pennies",
        CASE
            WHEN ("l"."delivery" = 'online'::"public"."delivery") THEN ("round"(((("l"."duration_min" * "str"."effective_online_rate_pennies"))::numeric / (60)::numeric)))::integer
            WHEN ("l"."delivery" = 'f2f'::"public"."delivery") THEN ("round"(((("l"."duration_min" * "str"."effective_f2f_rate_pennies"))::numeric / (60)::numeric)))::integer
            ELSE NULL::integer
        END AS "gross_pennies"
   FROM ("public"."lessons" "l"
     JOIN "public"."v_student_teacher_rate_summary" "str" ON ((("str"."student_id" = "l"."student_id") AND ("str"."teacher_id" = "l"."teacher_id"))))
  WHERE (("l"."state" = 'confirmed'::"public"."lesson_state") AND ((("l"."delivery" = 'online'::"public"."delivery") AND ("str"."effective_online_rate_pennies" IS NOT NULL)) OR (("l"."delivery" = 'f2f'::"public"."delivery") AND ("str"."effective_f2f_rate_pennies" IS NOT NULL))));


ALTER VIEW "public"."v_teacher_lesson_earnings_detail" OWNER TO "postgres";


CREATE OR REPLACE VIEW "public"."v_lesson_margin_detail" WITH ("security_invoker"='true') AS
 SELECT "e"."lesson_id",
    "e"."teacher_id",
    "e"."student_id",
    "e"."start_at",
...
CREATE OR REPLACE VIEW "public"."v_teacher_drinks_expenses_by_student_month" WITH ("security_invoker"='true') AS
 SELECT "teacher_id",
    "student_id",
...
CREATE OR REPLACE VIEW "public"."v_lesson_margin_with_drinks_detail" AS
 WITH "lesson_totals" AS (
         SELECT "v_lesson_margin_detail"."teacher_id",
            "v_lesson_margin_detail"."student_id",
            ("date_trunc"('month'::"text", "timezone"('Europe/London'::"text", "v_lesson_margin_detail"."start_at")))::"date" AS "month_start",
            "sum"(
                CASE
                    WHEN ("v_lesson_margin_detail"."delivery" = 'f2f'::"public"."delivery") THEN "v_lesson_margin_detail"."duration_min"
                    ELSE 0
                END) AS "total_minutes"
           FROM "public"."v_lesson_margin_detail"
          GROUP BY "v_lesson_margin_detail"."teacher_id", "v_lesson_margin_detail"."student_id", (("date_trunc"('month'::"text", "timezone"('Europe/London'::"text", "v_lesson_margin_detail"."start_at")))::"date")
        )
 SELECT "m"."lesson_id",
    "m"."teacher_id",
    "m"."student_id",
    ...
    "m"."margin_pennies" AS "margin_before_drinks_pennies",
        CASE
            WHEN (("m"."delivery" = 'f2f'::"public"."delivery") AND ("lt"."total_minutes" IS NOT NULL) AND ("lt"."total_minutes" > 0) AND ("de"."drinks_approved_pennies" IS NOT NULL)) THEN ("round"(((("m"."duration_min")::numeric * ("de"."drinks_approved_pennies")::numeric) / ("lt"."total_minutes")::numeric)))::integer
            ELSE 0
    ...
    ("m"."margin_pennies" -
        CASE
            WHEN (("m"."delivery" = 'f2f'::"public"."delivery") AND ("lt"."total_minutes" IS NOT NULL) AND ("lt"."total_minutes" > 0) AND ("de"."drinks_approved_pennies" IS NOT NULL)) THEN ("round"(((("m"."duration_min")::numeric * ("de"."drinks_approved_pennies")::numeric) / ("lt"."total_minutes")::numeric)))::integer
            ELSE 0
        END) AS "margin_after_drinks_pennies",
        CASE
            WHEN ("m"."revenue_pennies" > 0) THEN (((("m"."margin_pennies" -
            CASE
                WHEN (("m"."delivery" = 'f2f'::"public"."delivery") AND ("lt"."total_minutes" IS NOT NULL) AND ("lt"."total_minutes" > 0) AND ("de"."drinks_approved_pennies" IS NOT NULL)) THEN ("round"(((("m"."duration_min")::numeric * ("de"."drinks_approved_pennies")::numeric) / ("lt"."total_minutes")::numeric)))::integer
                ELSE 0
            END))::numeric * 100.0) / ("m"."revenue_pennies")::numeric)
            ELSE NULL::numeric
        END AS "margin_after_drinks_pct",
    ("date_trunc"('month'::"text", "timezone"('Europe/London'::"text", "m"."start_at")))::"date" AS "month_start",
        CASE
            WHEN ("m"."duration_min" = 60) THEN '60'::"public"."length_cat"
            WHEN ("m"."duration_min" = 90) THEN '90'::"public"."length_cat"
            WHEN ("m"."duration_min" = 120) THEN '120'::"public"."length_cat"
...
CREATE OR REPLACE VIEW "public"."v_teacher_names" WITH ("security_invoker"='true') AS
...
CREATE OR REPLACE VIEW "public"."v_lesson_margin_with_drinks_with_names" WITH ("security_invoker"='true') AS
 SELECT "m"."lesson_id",
    "m"."teacher_id",
    "m"."student_id",
    "m"."start_at",
...
CREATE OR REPLACE VIEW "public"."v_student_last_activity" WITH ("security_invoker"='true') AS
...
CREATE OR REPLACE VIEW "public"."v_past_students_cleanup_candidates" WITH ("security_invoker"='true') AS
 SELECT "s"."id" AS "student_id",
    "a"."last_activity_at",
...
CREATE OR REPLACE VIEW "public"."v_student_award_reason_summary" WITH ("security_invoker"='true') AS
 SELECT "student_id",
    "award_reason_code",
...
CREATE OR REPLACE VIEW "public"."v_student_credit_delivery_summary" WITH ("security_invoker"='true') AS
 WITH "base" AS (
         SELECT "v_credit_lot_remaining"."student_id",
            "v_credit_lot_remaining"."source_type",
            "v_credit_lot_remaining"."delivery_restriction",
            "v_credit_lot_remaining"."minutes_granted",
            "v_credit_lot_remaining"."minutes_allocated",
            "v_credit_lot_remaining"."minutes_remaining"
           FROM "public"."v_credit_lot_remaining"
        )
 SELECT "student_id",
...
CREATE OR REPLACE VIEW "public"."v_student_credit_summary" WITH ("security_invoker"='true') AS
 WITH "s" AS (
         SELECT "v_credit_lot_remaining"."student_id",
            (COALESCE("sum"("v_credit_lot_remaining"."minutes_granted"), (0)::bigint))::integer AS "total_granted_min",
            (COALESCE("sum"("v_credit_lot_remaining"."minutes_allocated"), (0)::bigint))::integer AS "total_allocated_min",
            (COALESCE("sum"("v_credit_lot_remaining"."minutes_remaining"), (0)::bigint))::integer AS "total_remaining_min",
            "min"(
                CASE
                    WHEN (("v_credit_lot_remaining"."expiry_date" IS NOT NULL) AND ("v_credit_lot_remaining"."expiry_date" >= CURRENT_DATE)) THEN "v_credit_lot_remaining"."expiry_date"
                    ELSE NULL::"date"
                END) AS "next_expiry_date"
           FROM "public"."v_credit_lot_remaining"
          GROUP BY "v_credit_lot_remaining"."student_id"
        )
...
CREATE OR REPLACE VIEW "public"."v_student_usage_last_3m" WITH ("security_invoker"='true') AS
 WITH "monthly" AS (
         SELECT "lessons"."student_id",
            "date_trunc"('month'::"text", "lessons"."occurred_at") AS "month_start",
            "sum"("lessons"."duration_min") AS "minutes_taken"
           FROM "public"."lessons"
          WHERE (("lessons"."state" = 'confirmed'::"public"."lesson_state") AND ("lessons"."occurred_at" >= "date_trunc"('month'::"text", ("now"() - '3 mons'::interval))))
          GROUP BY "lessons"."student_id", ("date_trunc"('month'::"text", "lessons"."occurred_at"))
        ), "agg" AS (
         SELECT "monthly"."student_id",
            "count"(*) AS "months_count",
            "sum"("monthly"."minutes_taken") AS "minutes_last_3m",
            "avg"("monthly"."minutes_taken") AS "avg_month_minutes"
           FROM "monthly"
          GROUP BY "monthly"."student_id"
        )
 SELECT "student_id",
    "months_count",
    "minutes_last_3m",
...
CREATE OR REPLACE VIEW "public"."v_student_dynamic_credit_alerts" WITH ("security_invoker"='true') AS
 SELECT "cs"."student_id",
    ...
    (("u"."avg_month_hours" IS NOT NULL) AND ("u"."avg_month_hours" > (0)::numeric) AND (((("cs"."total_remaining_min")::numeric / 60.0) - "u"."avg_month_hours") < 4.0)) AS "is_dynamic_low",
    (("cs"."total_remaining_min" <= 360) OR (("u"."avg_month_hours" IS NOT NULL) AND ("u"."avg_month_hours" > (0)::numeric) AND (((("cs"."total_remaining_min")::numeric / 60.0) - "u"."avg_month_hours") < 4.0))) AS "is_low_any"
...
CREATE OR REPLACE VIEW "public"."v_student_dynamic_credit_alerts_by_delivery" WITH ("security_invoker"='true') AS
...
CREATE OR REPLACE VIEW "public"."v_student_lifecycle_summary" WITH ("security_invoker"='true') AS
...
CREATE OR REPLACE VIEW "public"."v_student_snc_lessons" WITH ("security_invoker"='true') AS
 SELECT "id" AS "lesson_id",
    "student_id",
    "teacher_id",
    "occurred_at",
   ...
   FROM "public"."lessons" "l"
  WHERE (("is_snc" = true) AND ("state" = 'confirmed'::"public"."lesson_state"));


ALTER VIEW "public"."v_student_snc_lessons" OWNER TO "postgres";


CREATE OR REPLACE VIEW "public"."v_student_snc_status_by_month" WITH ("security_invoker"='true') AS
 SELECT "student_id",
...
CREATE OR REPLACE VIEW "public"."v_student_snc_status_previous_month" WITH ("security_invoker"='true') AS
 SELECT "student_id",
    "free_sncs",
    "charged_sncs",
    "has_free_snc_used"
...
CREATE OR REPLACE VIEW "public"."v_teacher_expenses_summary" WITH ("security_invoker"='true') AS
 SELECT "teacher_id",
    ("date_trunc"('month'::"text", "timezone"('Europe/London'::"text", "incurred_at")))::"date" AS "month_start",
    "sum"(
        CASE
...
CREATE OR REPLACE VIEW "public"."v_teacher_lesson_earnings_by_month" WITH ("security_invoker"='true') AS
 SELECT "teacher_id",
...
CREATE OR REPLACE VIEW "public"."v_teacher_invoice_summary" WITH ("security_invoker"='true') AS
 WITH "month_keys" AS (
        ...
        UNION
         SELECT "v_teacher_expenses_summary"."teacher_id",
            "v_teacher_expenses_summary"."month_start"
           FROM "public"."v_teacher_expenses_summary"
        UNION
         SELECT "teacher_invoices"."teacher_id",
            "teacher_invoices"."month_start"
           FROM "public"."teacher_invoices"
        ), "combined" AS (
...
CREATE OR REPLACE VIEW "public"."v_teacher_last_activity" WITH ("security_invoker"='true') AS
 SELECT "t"."id" AS "teacher_id",
...
CREATE OR REPLACE VIEW "public"."v_teacher_lesson_earnings_by_student_month" WITH ("security_invoker"='true') AS
 SELECT "d"."teacher_id",
    "d"."student_id",
    "sn"."display_name" AS "student_name",
...
CREATE OR REPLACE VIEW "public"."v_teacher_lesson_earnings_last_month" WITH ("security_invoker"='true') AS
 WITH "bounds" AS (
...
CREATE OR REPLACE VIEW "public"."v_teacher_lesson_margin_by_student_month" WITH ("security_invoker"='true') AS
 SELECT "m"."teacher_id",
    "m"."student_id",
    "sn"."display_name" AS "student_name",
    ("date_trunc"('month'::"text", "timezone"('Europe/London'::"text", "m"."start_at")))::"date" AS "month_start",
...
CREATE OR REPLACE VIEW "public"."v_teacher_lesson_revenue_by_month" WITH ("security_invoker"='true') AS
 SELECT "l"."teacher_id",
...
CREATE OR REPLACE VIEW "public"."v_teacher_lesson_stats_by_month" WITH ("security_invoker"='true') AS
 SELECT "teacher_id",
    ("date_trunc"('month'::"text", "timezone"('Europe/London'::"text", "occurred_at")))::"date" AS "month_start",
    "count"(*) AS "lesson_count_total",
...
CREATE OR REPLACE VIEW "public"."v_teacher_lessons" WITH ("security_invoker"='true') AS
 SELECT "l"."id",
    "l"."teacher_id",
    "l"."student_id",
    "l"."occurred_at" AS "start_at",
...
CREATE OR REPLACE VIEW "public"."v_teacher_margin_by_month" WITH ("security_invoker"='true') AS
 WITH "revenue" AS (
 ...
 SELECT COALESCE("r"."teacher_id", "e"."teacher_id", "x"."teacher_id") AS "teacher_id",
    ...
    (COALESCE("r"."revenue_pennies", (0)::bigint) - COALESCE("e"."gross_pennies", (0)::bigint)) AS "margin_before_expenses_pennies",
    ((COALESCE("r"."revenue_pennies", (0)::bigint) - COALESCE("e"."gross_pennies", (0)::bigint)) - COALESCE("x"."expenses_approved_pennies", (0)::bigint)) AS "margin_after_expenses_pennies",
        CASE
            WHEN (COALESCE("r"."revenue_pennies", (0)::bigint) > 0) THEN ((((COALESCE("r"."revenue_pennies", (0)::bigint) - COALESCE("e"."gross_pennies", (0)::bigint)))::numeric * 100.0) / (COALESCE("r"."revenue_pennies", (0)::bigint))::numeric)
            ELSE NULL::numeric
        END AS "margin_before_expenses_pct",
        CASE
            WHEN (COALESCE("r"."revenue_pennies", (0)::bigint) > 0) THEN (((((COALESCE("r"."revenue_pennies", (0)::bigint) - COALESCE("e"."gross_pennies", (0)::bigint)) - COALESCE("x"."expenses_approved_pennies", (0)::bigint)))::numeric * 100.0) / (COALESCE("r"."revenue_pennies", (0)::bigint))::numeric)
            ELSE NULL::numeric
...
CREATE OR REPLACE VIEW "public"."v_teacher_margin_by_month_with_names" WITH ("security_invoker"='true') AS
...
CREATE OR REPLACE VIEW "public"."v_teacher_rate_summary" WITH ("security_invoker"='true') AS
...
CREATE OR REPLACE VIEW "public"."v_teacher_usage_last_3m" WITH ("security_invoker"='true') AS
 WITH "monthly" AS (
         SELECT "l"."teacher_id",
            "date_trunc"('month'::"text", "l"."occurred_at") AS "month_start",
...
CREATE OR REPLACE VIEW "public"."v_credit_lot_remaining" WITH ("security_invoker"='true') AS
 SELECT "cl"."id" AS "credit_lot_id",
    "cl"."student_id",
    "cl"."source_type",
    ...
    "cl"."external_ref",
    "cl"."minutes_granted",
    (COALESCE("sum"("a"."minutes_allocated"), (0)::bigint))::integer AS "minutes_allocated",
    (("cl"."minutes_granted" - COALESCE("sum"("a"."minutes_allocated"), (0)::bigint)))::integer AS "minutes_remaining",
    (("cl"."minutes_granted" - COALESCE("sum"("a"."minutes_allocated"), (0)::bigint)) < 0) AS "is_overdrawn",
    "cl"."delivery_restriction",
    "cl"."tier_restriction",
    "cl"."length_restriction",
    "cl"."start_date",
    ...
    "cl"."expiry_date",
        CASE
            WHEN ("cl"."expiry_date" IS NULL) THEN NULL::integer
            ELSE ("cl"."expiry_date" - CURRENT_DATE)
        END AS "days_to_expiry",
    (("cl"."expiry_date" IS NOT NULL) AND ("cl"."expiry_date" >= CURRENT_DATE) AND ("cl"."expiry_date" <= (CURRENT_DATE + 30))) AS "expiry_within_30d",
        CASE
            WHEN (("cl"."state" = 'open'::"public"."credit_lot_state") AND (("cl"."minutes_granted" - COALESCE("sum"("a"."minutes_allocated"), (0)::bigint)) <= 0)) THEN 'closed'::"public"."credit_lot_state"
            ELSE "cl"."state"
        END AS "state",
    "cl"."created_at"
   FROM ("public"."credit_lots" "cl"
...
CREATE POLICY "allocations admin write" ON "public"."allocations" USING ("public"."auth_is_admin"()) WITH CHECK ("public"."auth_is_admin"());



CREATE POLICY "allocations select" ON "public"."allocations" FOR SELECT USING (("public"."auth_is_admin"() OR (EXISTS ( SELECT 1
   FROM "public"."lessons" "l"
  WHERE (("l"."id" = "allocations"."lesson_id") AND ((EXISTS ( SELECT 1
           FROM "public"."teachers" "t"
          WHERE (("t"."id" = "l"."teacher_id") AND ("t"."profile_id" = "auth"."uid"())))) OR (EXISTS ( SELECT 1
           FROM "public"."students" "s"
          WHERE (("s"."id" = "l"."student_id") AND ("s"."profile_id" = "auth"."uid"())))))))) OR (EXISTS ( SELECT 1
   FROM "public"."credit_lots" "cl"
  WHERE (("cl"."id" = "allocations"."credit_lot_id") AND ((EXISTS ( SELECT 1
           FROM "public"."students" "s"
          WHERE (("s"."id" = "cl"."student_id") AND ("s"."profile_id" = "auth"."uid"())))) OR (EXISTS ( SELECT 1
           FROM ("public"."student_teacher" "st"
             JOIN "public"."teachers" "t" ON (("t"."id" = "st"."teacher_id")))
          WHERE (("st"."student_id" = "cl"."student_id") AND ("t"."profile_id" = "auth"."uid"()))))))))));



CREATE POLICY "any_authenticated_can_select_profiles" ON "public"."profiles" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "any_authenticated_can_select_teachers" ON "public"."teachers" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "credit_lots select" ON "public"."credit_lots" FOR SELECT USING (("public"."auth_is_admin"() OR (EXISTS ( SELECT 1
   FROM "public"."students" "s"
...
CREATE POLICY "credit_lots write admin" ON "public"."credit_lots" USING ("public"."auth_is_admin"()) WITH CHECK ("public"."auth_is_admin"());
...
CREATE POLICY "lessons admin write" ON "public"."lessons" USING ("public"."auth_is_admin"()) WITH CHECK ("public"."auth_is_admin"());



CREATE POLICY "lessons insert by teacher or admin" ON "public"."lessons" FOR INSERT WITH CHECK (("public"."auth_is_admin"() OR ((EXISTS ( SELECT 1
   FROM "public"."teachers" "t"
...
CREATE POLICY "profiles read assigned students for teacher" ON "public"."profiles" FOR SELECT USING ((("id" = "auth"."uid"()) OR "public"."auth_is_admin"() OR (EXISTS ( SELECT 1
   FROM (("public"."students" "s"
     JOIN "public"."student_teacher" "st" ON (("st"."student_id" = "s"."id")))
     JOIN "public"."teachers" "t" ON (("t"."id" = "st"."teacher_id")))
  WHERE (("s"."profile_id" = "profiles"."id") AND ("t"."profile_id" = "auth"."uid"()))))));



CREATE POLICY "profiles self read" ON "public"."profiles" FOR SELECT USING ((("id" = "auth"."uid"()) OR "public"."auth_is_admin"()));



CREATE POLICY "profiles self update" ON "public"."profiles" FOR UPDATE USING ((("id" = "auth"."uid"()) OR "public"."auth_is_admin"()));
...
CREATE POLICY "student_teacher readable (admin/teacher/self)" ON "public"."student_teacher" FOR SELECT USING (("public"."auth_is_admin"() OR (EXISTS ( SELECT 1
   FROM "public"."teachers" "t"
  WHERE (("t"."id" = "student_teacher"."teacher_id") AND ("t"."profile_id" = "auth"."uid"())))) OR "public"."is_current_student"("student_id")));



CREATE POLICY "students read own or admin" ON "public"."students" FOR SELECT USING ((("profile_id" = "auth"."uid"()) OR "public"."auth_is_admin"()));



CREATE POLICY "students readable (self/admin/assigned-teacher)" ON "public"."students" FOR SELECT USING (("public"."auth_is_admin"() OR ("profile_id" = "auth"."uid"()) OR "public"."is_teacher_assigned_to_student"("id")));
...
CREATE POLICY "teacher can read their students" ON "public"."students" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM ("public"."lessons" "l"
     JOIN "public"."teachers" "t" ON (("t"."id" = "l"."teacher_id")))
  WHERE (("l"."student_id" = "students"."id") AND ("t"."profile_id" = "auth"."uid"())))));
...
CREATE POLICY "teacher_expenses_delete_pending_own" ON "public"."teacher_expenses" FOR DELETE USING ((("status" = 'pending'::"text") AND ("teacher_id" IN ( SELECT "t"."id"
   FROM "public"."teachers" "t"
  WHERE ("t"."profile_id" = "auth"."uid"())))));
...
CREATE POLICY "teachers read own lessons" ON "public"."lessons" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."teachers" "t"
  WHERE (("t"."id" = "lessons"."teacher_id") AND ("t"."profile_id" = "auth"."uid"())))));



CREATE POLICY "teachers read own or admin" ON "public"."teachers" FOR SELECT USING ((("profile_id" = "auth"."uid"()) OR "public"."auth_is_admin"()));
...
CREATE POLICY "teachers_can_insert_own_lessons" ON "public"."lessons" FOR INSERT TO "authenticated" WITH CHECK (("teacher_id" IN ( SELECT "t"."id"
   FROM "public"."teachers" "t"
  WHERE ("t"."profile_id" = "auth"."uid"()))));
...
CREATE POLICY "teachers_can_select_self" ON "public"."teachers" FOR SELECT TO "authenticated" USING (("profile_id" = "auth"."uid"()));
...
GRANT ALL ON FUNCTION "public"."rpc_import_invoice"("p_student_id" "uuid", "p_external_ref" "text", "p_minutes_granted" integer, "p_amount_pennies" integer, "p_start_date" "date", "p_delivery_restriction" "text", "p_tier_restriction" "text", "p_length_restriction" "public"."length_cat", "p_expiry_date" "date", "p_expiry_policy" "public"."expiry_policy", "p_lessons_per_month" integer, "p_duration_per_lesson_mins" integer, "p_buffer" numeric) TO "anon";
GRANT ALL ON FUNCTION "public"."rpc_import_invoice"("p_student_id" "uuid", "p_external_ref" "text", "p_minutes_granted" integer, "p_amount_pennies" integer, "p_start_date" "date", "p_delivery_restriction" "text", "p_tier_restriction" "text", "p_length_restriction" "public"."length_cat", "p_expiry_date" "date", "p_expiry_policy" "public"."expiry_policy", "p_lessons_per_month" integer, "p_duration_per_lesson_mins" integer, "p_buffer" numeric) TO "authenticated";
GRANT ALL ON FUNCTION "public"."rpc_import_invoice"("p_student_id" "uuid", "p_external_ref" "text", "p_minutes_granted" integer, "p_amount_pennies" integer, "p_start_date" "date", "p_delivery_restriction" "text", "p_tier_restriction" "text", "p_length_restriction" "public"."length_cat", "p_expiry_date" "date", "p_expiry_policy" "public"."expiry_policy", "p_lessons_per_month" integer, "p_duration_per_lesson_mins" integer, "p_buffer" numeric) TO "service_role";
